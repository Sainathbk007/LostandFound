...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $student_college_id = trim($_POST['student_college_id'] ?? '');
    ...
    $message = 'Please provide item name, contact info, and your college ID.';
  } else {
    $imageFilename = null;
    ...
    $stmt = $mysqli->prepare("INSERT INTO lost_items (item_name, description, date_lost, location, contact_info, student_college_id, image, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'open')");
    $stmt->bind_param('sssssss', $item_name, $description, $date_lost, $location, $contact_info, $student_college_id, $imageFilename);
    $ok = $stmt->execute();
    $stmt->close();
    if ($ok) {
      $message = 'success'; // used for toast
    } else {
...
<div class="container py-4">
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      <label class="form-label">Student College ID</label>
      <input name="student_college_id" class="form-control" required>
    </div>
    <div class="mb-3">
  ...
  <div id="successToast" class="toast align-items-center text-bg-success border-0" role="alert" aria-live="assertive" aria-atomic="true">
    <div class="d-flex">
      <div class="toast-body">
        Your lost item report was submitted.
      </div>
